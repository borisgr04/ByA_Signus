var objregistro = (function () {
    ...
    var _crearElements = function () {
        var now = new Date();
        $("#txtFechaPago").val(byaPage.FechaShortX(now));

        $("#txtBsqVigenciaLiquidacion").val(byaSite.getVigencia());
        $("#secInfo").msgBox({ titulo: "", mensaje: "<strong>Ayuda:</strong> Digite en número de liquidación para realizar el pago", tipo: 'info' });
        $(".habilitarpago").byaSetHabilitar(false);
        $("#txtBsqNumeroLiquidacion").focus();
    };
    var _traerLiquidacion = function () {
        SL_LIQESTAMPILLAS_DAO.GetXNroLiq($("#txtBsqNumeroLiquidacion").val(), $("#txtBsqVigenciaLiquidacion").val(), function (result) {
            ...
            if (Liq != null) {
                ...
                if (Liq.ESTADO == "LI") { Liq.NOM_ESTADO = "LIQUIDADO"; $(".habilitarpago").byaSetHabilitar(true); }
                if (Liq.ESTADO == "PA") { Liq.NOM_ESTADO = "PAGADO"; $(".habilitarpago").byaSetHabilitar(false); }
                if (Liq.ESTADO == "IN") { Liq.NOM_ESTADO = "ANULADO"; $(".habilitarpago").byaSetHabilitar(false); }
                ...
                var DataFields = [
                        ...
                        { Titulo: 'Reportada', Campo: 'REPLICADO', Tipo: 'S' },
                        { Titulo: 'Periodo', Campo: 'PER_LIQ', Tipo: 'S' },
                        { Titulo: 'Fecha liquidación', Campo: 'FEC_LIQ', Tipo: 'S' },
                        ...
                        { Titulo: 'Observación', Campo: 'OBS_LIQ', Tipo: 'S' }
    ...
    var RealizarPago = function () {
        if (($("#txtFechaPago").val() == "") || ($("#txtValorPago").val() == "")) {
            alert("Debe completar los campos");
        } else {
            var e = {};
            e.ID_LIQ = Liq.ID;
            e.FEC_PAGO = $("#txtFechaPago").val();
            e.VAL_PAGO = $("#txtValorPago").byaGetDecimal();

            if (e.VAL_PAGO == ValorPagar) {
                SL_PAGOS_DAO.Insert(e, function (result) {
                    var Res = byaPage.convertOBJ(result);
                    $("#secInfo").msgBox({ titulo: "Pagos", mensaje: Res.Mensaje, tipo: !Res.Error });
                    byaPage.irInicio();
                    if (Res.Error == false) { posPago = true;  _traerLiquidacion(); }
                });
            } else {
                alert("El valor a pagar no coincide con el valor de la liquidación");
            }
        }
    };
    return {
...
$(function () {
    byaSite.SetModuloP({ TituloForm: "Pagos", Modulo: "Gestión", urlToPanelModulo: "Pagos.aspx", Cod_Mod: "SLQES", Rol: "SLQESPagos" });
    objregistro.init();
});
