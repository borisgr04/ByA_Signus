var objregistro = (function () {
    ...
    var _traerLiquidacion = function () {
        SL_LIQESTAMPILLAS_DAO.GetXNroLiq($("#txtBsqNumeroLiquidacion").val(), $("#txtBsqVigenciaLiquidacion").val(), function (result) {
            ...
            if (Liq != null) {
                ...
                if (Liq.ESTADO == "LI") { Liq.NOM_ESTADO = "LIQUIDADO"; $(".habilitarcancelar").byaSetHabilitar(false); }
                if (Liq.ESTADO == "PA") { Liq.NOM_ESTADO = "PAGADO"; $(".habilitarcancelar").byaSetHabilitar(true); }
                ...
                if (!posPago) {
                    if (Liq.ESTADO == "IN") $("#secInfo").msgBox({ titulo: "", mensaje: "Se cargó la liquidación Nro.  " + $("#txtBsqNumeroLiquidacion").val() + " de " + $("#txtBsqVigenciaLiquidacion").val() + ", <strong>esta liquidación fue anulada</strong><br/>Pulse en <strong>+ Información de la liquidación</strong> para ver detalles", tipo: 'danger' });
                        ...
                        if (Liq.ESTADO != "PA") $("#secInfo").msgBox({ titulo: "", mensaje: "Se cargó la liquidación Nro.  " + $("#txtBsqNumeroLiquidacion").val() + " de " + $("#txtBsqVigenciaLiquidacion").val() + ", <strong>esta liquidación no ha sido pagada</strong><br/>Pulse en <strong>+ Información de la liquidación</strong> para ver detalles", tipo: 'danger' });
    ...
    var CancelarPago = function () {
        SL_PAGOS_DAO.CancelarPago(Liq.ID, function (result) {
            var Res = byaPage.convertOBJ(result);
            $("#secInfo").msgBox({ titulo: "Cancelar Pago", mensaje: Res.Mensaje, tipo: !Res.Error });
...
$(function () {
    byaSite.SetModuloP({ TituloForm: "Cancelar Pago", Modulo: "Gestión", urlToPanelModulo: "Pagos.aspx", Cod_Mod: "SLQES", Rol: "SLQESCanPag" });
    objregistro.init();
});
